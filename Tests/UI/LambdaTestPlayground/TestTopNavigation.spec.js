...
test('Navigate through the site using the top navigation bar.', runTest(async (page) => {
    ...
    const loginOption = await navigationMenu.isLoginDisplayed();
    const registerOption = await navigationMenu.isRegisterDisplayed();
    ...
    if (loginOption && registerOption) { // if user is not logged in
        ...
        const loginOptionAfterLogout = await navigationMenu.isLoginDisplayed();
        expect(loginOptionAfterLogout).toBe(true, 'Login option in My account should be displayed after user logs out.');
    }
    ...
    await navigationMenu.clickLogoutOptionInMyAccount();
    await navigationMenu.hoverMyAccountNav();
    const loginOptionAfterSecondLogout = await navigationMenu.isLoginDisplayed();
    expect(loginOptionAfterSecondLogout).toBe(true, 'Login option in My account should be displayed after user logs out.');
}));
